...
export class MigrationController {
  ...
  @HttpCode(HttpStatus.OK)
  async prepareCreateFromSource(
    @Body() body: CreateMigrationFromSourceDto,
    @Req() req: any,
  ) {
    return this.governance.prepare({
      module: 'migrations',
  ...
  async confirmCreateFromSource(
    ...
    try {
      ...
      const created = await this.migrations.createJobWithAccounts(
        input,
        accounts,
      );
      const result = {
  ...
  async prepareAddAccount(
    @Param('id') id: string,
    @Body() body: AddMigrationAccountDto & { reason?: string },
  ) {
  ...
  async confirmAddAccount(
    ...
    try {
      const result = await this.migrations.addAccount(id, payload);
      return this.governance.recordResult({
        intent,
        status: 'SUCCESS',
        steps,
  ...
  async confirmPlan(
    ...
    if ((intent.payload as any)?.id !== id)
  ...
  async confirmStart(
    ...
    if ((intent.payload as any)?.id !== id)
      throw new Error('Intent target mismatch');
    const steps: ActionStep[] = [
      { name: 'start_background_migration', status: 'SUCCESS' },
    ];
    try {
      ...
      const result = {
        ...
        poll: {
          jobId: id,
          stepsEndpoint: `/v1/migrations/${id}/steps`,
          logsEndpoint: `/v1/migrations/${id}/logs`,
        },
      };
      return this.governance.recordResult({
        intent,
      ...
      steps[0] = {
        name: 'start_background_migration',
        status: 'FAILED',
        errorMessage: e instanceof Error ? e.message : 'unknown_error',
      };
      return this.governance.recordResult({
